...
impl Audio {
    ...
    pub fn timestretch(
        ...
        // start immediately TODO later
        gain_node.connect(&*self.register_type(r#type).lock().unwrap().gain);

        Ok(StreamingSoundEffect {
            r#type,
            context: self.clone(),
            source_node: worklet,
            fade_node,
...
impl SoundEffect {
    ...
    pub fn play_at_from(&mut self, start_delay: time::Duration, offset: time::Duration) {
        let node: &dyn wa::AudioNode = match &self.spatial_state {
            SpatialState::NotSpatial => &self.gain_node,
            SpatialState::Spatial(panner) => panner,
        };
        node.connect(&*self.context.register_type(self.r#type).lock().unwrap().gain);
        self.source_node
            .start_at_with_offset(start_delay.as_secs_f64(), offset.as_secs_f64());
    }
